...
For establishing a communication between the two components, the Thoughtram article made use of the powerful **hierarchical dependency injector** which is available in Angular 2. In the `<tab>` child component it simply asked for its parent `<tabs>` and registered itself on that parent component, using the `addTab` function:
...
> 
We can take totally different approach how to implement our simple tabs ( which isn’t so easily possible in Angular 1 ), leveraging special Angular 2 @ContentChildren property decorator with QueryList type and AfterContentInit life cycle interface. Those are more advanced concepts, which we will cover in future articles.

## The alternative approach

Well, this article is just about continuing Thoughtram's example, but without using the hierarchical dependency injector. So basically, rather than getting a reference to our parent component `Tabs` in the child component `Tab` (`child => parent`), we're doing it the other way round: we'll get a reference to all of the `Tab` child components from the parent `Tabs` (`parent => child`).
